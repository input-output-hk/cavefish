...
$$\Pr[ (ek,dk) \gets \Keygen(1^\lambda); m\gets \mathcal{M}:\Dec(sk,\Enc(pk,m))=m ] =1.$$
\end{definition}
...
$$\left| \Pr[ (ek,dk) \gets \Keygen(1^\lambda); (m_0,m_1)\gets A(ek);d\gets\{0,1\}; c^*\gets \Enc(ek,m_d):A(c^*)=d ] - {1\over2} \right|$$
