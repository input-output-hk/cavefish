...
These functions are defined such that for any $l,~i$, where $\fun{mkToSpec}~l~i~\var{tx}_t \neq \fun{nothing}$,
necessarily $\chkSpec{i}~(\fun{mkAbs}~(\fun{mkToSpec}~l~i~\var{tx}_t))$,
see Appendix~\ref{sec:appendix} for a proof sketch.\textcolor{orange}{mr: don't think we provide a proof? We need to motivate the DSL?}
Depending on the functionality of the light client, more constraints need to be
